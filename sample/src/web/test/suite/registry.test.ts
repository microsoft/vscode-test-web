...
suite('Playwright registry management', () => {
	...
	test('generates 3 handles in a test', async function() {
		...
		const sizeBefore = await (playwright as any).__registrySize();
		...
		const sizeAfter = await (playwright as any).__registrySize();
	...
	test('manual clear resets registry to 0', async function() {
		...
		assert.strictEqual(await (playwright as any).__registrySize(), sizeBefore + 2);
		await (playwright as any).__clearRegistry();
		assert.strictEqual(await (playwright as any).__registrySize(), 0, 'Registry should be 0 after manual clear');
	});

	test('registry cleared between tests (size == 0)', async function() {
		this.timeout(5000);
		const sizeNow = await (playwright as any).__registrySize();
